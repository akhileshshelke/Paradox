...
curl -s localhost:8001/api/stations | jq '.[0]'
curl -s localhost:8001/api/forecast/anand_vihar | jq
curl -s localhost:8001/api/recommendations | jq '.regime, (.recommendations | length)'
```
...
async def go():
    async with aiohttp.ClientSession() as s, s.ws_connect('ws://localhost:8001/ws/live') as ws:
        async for m in ws:
